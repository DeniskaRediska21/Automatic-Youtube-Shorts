...
for i in range(len(topics)):
    ...
    prompts = responce_split


    images = []

    for i, prompt in enumerate(prompts):
        image = pipe(Title_+'. '+prompt+' Photorealistic, high contrast.',height=1024, width = 768).images[0]
        image.save(savePath+f'{i}.jpg')
        images.append(image) 
    ...
    for i, prompt in enumerate(prompts):
        ...
        image.save(savePath+f'{i}_ref.jpg')
    ...
    for i in range(len(img_array)):
        out = cv2.VideoWriter(savePath+f'{i}.avi',cv2.VideoWriter_fourcc(*'DIVX'), 1/duration[i], size)
        out.write(cv2.cvtColor(img_array[i], cv2.COLOR_RGB2BGR))
        video_clips.append(savePath+f'{i}.avi')
    out.release()
    loaded_video_list = []
    # pyMovie
    ...
    for i,video_file in enumerate(video_clips):
        ...
        # os.system(f'ffmpeg -i -y {video_file} -i {audio_file} -map 0:v -map 1:a -c:v copy -shortest {savePath+str(i)+"_with_sound.mp4"}')
